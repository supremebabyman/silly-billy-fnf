...
function onCreate()
    ...
    if not lowQuality then
        ...
        makeLuaSprite('Silly_clouds', 'Silly_clouds', -800, -900);
        setScrollFactor('Silly_clouds', 1.0, 1.0);
        scaleObject('Silly_clouds', 1.0, 1.0);
    end

    -- Add the sprites based on their layering
    addLuaSprite('illmakeyoushake', false);
    if not lowQuality then
        -- Add the foreground sprites first
        addLuaSprite('pillarsforegroun', true);
        addLuaSprite('Silly_clouds', false);
    end
end
